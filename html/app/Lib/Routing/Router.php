 ...
 */
    ...
    // This has been heavily modified to allow us to route REST resources
    // based on versioning
	public static function mapResources($controller, $options = array()) {
		...
		foreach ((array)$controller as $name) {
			...
			foreach (self::$_resourceMap as $params) {
                ...
                // When version is set direct us to a special versioned controller
                if(isset($options['version'])){
                    Router::connect($url,
                        array(
                            'pass' => array("TESTING"),
                            'plugin' => $plugin,
                            'controller' => $urlName . 'v' . $options['version'],
